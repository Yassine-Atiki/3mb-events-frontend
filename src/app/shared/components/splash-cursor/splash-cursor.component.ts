...
/**
 * Effet "splash cursor" (simulation fluide WebGL) scopé à son conteneur.
 * Enveloppe ton contenu existant : <app-splash-cursor>...ton panneau...</app-splash-cursor>
 * L'effet ne réagit qu'au survol de CE conteneur, pas de toute la page.
 */
@Component({
  selector: 'app-splash-cursor',
  standalone: true,
  template: `
    <div class="splash-cursor-host">
      <canvas #canvas class="splash-cursor-canvas"></canvas>
      <div class="splash-cursor-content">
...
export class SplashCursorComponent implements AfterViewInit, OnDestroy {
  ...
  @Input() CURL = 3;
  /** Rayon des taches, plus petit = plus subtil dans un conteneur restreint. */
  @Input() SPLAT_RADIUS = 0.15;
  /** Force des taches, réduite par rapport à l'original (6000) pour un rendu plus sobre. */
  @Input() SPLAT_FORCE = 4000;
  @Input() SHADING = true;
  @Input() COLOR_UPDATE_SPEED = 10;
  @Input() RAINBOW_MODE = true;
  /** Utilisée si RAINBOW_MODE = false. */
  @Input() COLOR = '#10b981';
  /**
   * Utilisée si RAINBOW_MODE = true : les taches piochent aléatoirement
   * dans cette palette au lieu d'un arc-en-ciel complet -> reste dans
   * l'identité verte au lieu de couleurs hors charte.
   */
  @Input() BRAND_PALETTE: string[] = ['#10b981', '#14b8a6', '#34d399', '#059669', '#2dd4bf'];

  private animationFrameId: number | null = null;
  private hostListeners: Array<{ el: HTMLElement; type: string; fn: EventListener }> = [];
  ...
  ngOnDestroy(): void {
    ...
    for (const { el, type, fn } of this.hostListeners) {
  ...
  private initFluidSimulation(): void {
    ...
    const brandPalette = this.BRAND_PALETTE.length ? this.BRAND_PALETTE : [this.COLOR];
    ...
    // Palette de marque au lieu du hue arc-en-ciel d'origine.
    function generateColor(): ColorRGB {
      if (!config.RAINBOW_MODE) {
        return hexToRGB(config.COLOR);
      }
      const hex = brandPalette[Math.floor(Math.random() * brandPalette.length)];
      const base = hexToRGB(hex);
      const variation = 0.85 + Math.random() * 0.3;
      return { r: base.r * variation, g: base.g * variation, b: base.b * variation };
    }
    ...
    const addListener = (type: string, fn: EventListener, opts?: boolean | AddEventListenerOptions) => {
      hostEl.addEventListener(type, fn, opts);
      self.hostListeners.push({ el: hostEl, type, fn });
    };

    addListener('mousedown', (e: Event) => {
      const me = e as MouseEvent;
      const pointer = pointers[0];
      const { x, y } = getRelativePos(me.clientX, me.clientY);
